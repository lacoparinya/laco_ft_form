        ...
        <div class="row">
            @foreach ($data as $key => $item)
                @foreach ($item as $key2 => $item2)
                    <div class="col-md-12">
                        <canvas id="my_Chart{{ $key }}{{ str_replace('-', '', str_replace('#', '', $key2)) }}"
                            height=20vh" width="90vw"></canvas>
                    </div>
                @endforeach
            @endforeach
        </div>
    ...
    <script>
        @foreach ($data as $key => $item)
            @foreach ($item as $key2 => $item2)
                ...
                var ctx =
                document.getElementById('my_Chart{{ $key }}{{ str_replace('-', '', str_replace('#', '', $key2)) }}').getContext('2d');
